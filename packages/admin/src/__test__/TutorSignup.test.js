...
import { LoginPage ,TeacherSignup } from "./locator";
const puppeteer = require("puppeteer");
const pti = require("puppeteer-to-istanbul");
const { async } = require("q");

test("Tutor Sign up Free Trial", async () => {
  ...
  const browser = await puppeteer.launch(Config.launchOptions).then(async browser => {
    ...
    const [startAsATeacher] = await page.$x("//button[contains(text(),\"Start As Tutor\")]");
    ...
    await page.type(TeacherSignup.step1.email, TeacherSignup.step1.inputValue.tutorEmail);
